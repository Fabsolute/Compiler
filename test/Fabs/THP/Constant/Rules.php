<?php


namespace Fabs\THP\Constant;


class Rules
{
    const THP = 'thp';
    const CODE = 'code';
    const USING = 'use';
    const VALUE = 'value';
    const CHAIN = 'chain';
    const OPERAND = 'operand';
    const COMMAND = 'command';
    const CONSTANT = 'constant';
    const EQUALITY = 'equality';
    const VARIABLE = 'variable';
    const OPERATION = 'operation';
    const EXPRESSION = 'expression';
    const OBJECT_PAIR = 'object_pair';
    const STRING_JOIN = 'string_join';
    const ARRAY_ACCESS = 'array_access';
    const OBJECT_ACCESS = 'object_access';
    const FUNCTION_CALL = 'function_call';
    const ARRAY_DECLARATION = 'array_declaration';
    const OBJECT_DECLARATION = 'object_declaration';
    const STATIC_CLASS_ACCESS = 'static_class_access';
    const NAMESPACE_IDENTIFIER = 'namespace_identifier';
    const LOGICAL_OPERATION_PRIMARY = 'logical_operation_primary';
    const LOGICAL_OPERATION_SECONDARY = 'logical_operation_secondary';
}